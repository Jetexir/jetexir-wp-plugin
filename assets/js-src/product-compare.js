  ...
  $('.jetexir-product-compare-button').on('click', function (e) {
    ...
    $.post(
      Jetexir.ajaxUrl,
      {
        ...
        action: "jetexir_product_compare_add_remove",
        product_id: $this.data('id')
      }
    )
      .done(function (data) {
        if (data.data.status === 'added')
          $this.addClass('jetexir-button-remove');
        else if (data.data.status === 'removed')
          $this.removeClass('jetexir-button-remove');
        else if (data.data.status === 'max_exceeded')
          alert(JetexirProductCompare.maxExceededMessage.replace('%number%', data.data.count));
