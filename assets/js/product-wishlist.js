...
jQuery(document).ready(function ($) {
  $('.jetexir-product-wishlist-button').on('click', function (e) {
    ...
    const $this = $(this), jetexirWishlist = $this.attr('data-in-wishlist'),
      jetexirWishlistAddedAction = $this.attr('data-added-action');

    if (jetexirWishlist !== '' && jetexirWishlistAddedAction === 'open_page') {
    ...
    $.post(Jetexir.ajaxUrl, {
      nonce: Jetexir.ajaxNonce, action: "jetexir_product_wishlist_add_remove", product_id: $this.data('product-id')
    })
      .done(function (data) {
        ...
        } else if (data.data.status === 'max_exceeded') alert(JetexirProductWishlist.maxExceededMessage.replace('%number%', data.data.count));

        if ($this.data('action') === 'refresh') window.location.reload(true);
