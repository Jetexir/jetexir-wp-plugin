...
class Addons {
  ...
  public function flushRewriteRules(): void {
    if ( AdminPages::isSettingPage() &&
         Param::get( 'tab' ) === self::tab &&
         Param::get( 'addons-refreshed' ) === '1' &&
         current_user_can( 'manage_options' ) &&
         check_admin_referer( 'jetexir_flush_rewrite_rules' ) ) {
      flush_rewrite_rules();
  ...
  public function addRefreshNotice( $tab ): void {
    if ( $tab === self::tab && Cache::get( 'settings_saved' ) ) {
      ...
      add_filter( 'jetexir_settings_page_refresh_url', static function () {
        return wp_nonce_url(
          AdminPages::link( [ 'tab' => self::tab, 'addons-refreshed' => true ] ),
          'jetexir_flush_rewrite_rules'
        );
      } );
    }
  }
  ...
  public function settings(): array {
    /**
     * Filters the list of all Jetexir addons.
     *
     ...
     *
     */
    $addons    = (array) apply_filters( 'jetexir_addons', array() );
    $addonList = array();
  ...
  public static function getAddonCats(): ?array {
    ...
    /**
     * Filters the addon categories.
     *
     * @param array $cats Addon categories.
     *
     * @return array Addon categories.
     *
     * @since 1.0
     *
     */
    $cats = (array) apply_filters( 'jetexir_addon_cats', array() );
